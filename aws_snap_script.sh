...
EMAIL_LIST=youremail@fornotification.com #change for your e-mail
...
DESCRIPTION="Auto_${VOL_NAME}_${DATE}"

/usr/bin/aws ec2 create-snapshot --volume-id $VOL_ID --description "$DESCRIPTION" --region $REGION &>> $SNAP_CREATION
done
else
...
/usr/bin/aws ec2 describe-snapshots --query 'Snapshots[*].[SnapshotId,VolumeId,Description,StartTime]' --output text --filters "Name=status,Values=completed" "Name=volume-id,Values=$VOL_ID" | grep -v "CreateImage" > $SNAPSHOT_INFO
...
then
/usr/bin/aws ec2 delete-snapshot --snapshot-id $SNAP_ID --region $REGION --output text > /tmp/snap_del
